...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /**
     * Toggle the navbar's visual "scrolled" state based on the page vertical scroll position.
     *
     * Adds the `scrolled` class to the navbar when the vertical scroll is greater than 50 pixels;
     * removes the `scrolled` class when the scroll position is 50 pixels or less.
     */
    ...
    /**
     * Applies a vertical parallax translate to the page's hero element based on the current scroll position.
     *
     * If the global `hero` element exists, sets its inline `transform` to `translateY(<value>px)` where `<value>` is `window.pageYOffset * -0.05`.
     */
...
document.querySelectorAll('.btn').forEach(button => {
    ...
    /**
     * Build an SVG `<path>` string representing a smooth three-point curve where the middle point's y is adjustable.
     * @param {number} update - Y coordinate for the middle control point of the curve.
     * @param {number} smoothing - Smoothing factor that controls curve tension between points.
     * @return {string} An SVG `<path>` element string (`<path d="..."/>`) describing the smooth curve through the three points.
     */
    ...
    /**
     * Build a cubic Bezier path segment for `point` using neighbouring points and a smoothing factor.
     * @param {[number, number]} point - The target point as [x, y] coordinates for the segment's end.
     * @param {number} i - Index of the current point within the points array `a`.
     * @param {Array<[number, number]>} a - Array of points (each as [x, y]) used to compute control points.
     * @param {number} smoothing - Multiplier (typically between 0 and 1) that scales control handle lengths.
     * @returns {string} A SVG cubic Bezier command of the form `"C x1,y1 x2,y2 x,y"` for this segment.
     */
    ...
    /**
     * Creates a burst of animated particle dots inside a parent element and removes them after animation.
     *
     * Each particle is positioned at the provided (x, y) coordinates and animated outward with a random
     * angle between `minAngle` and `maxAngle` and a randomized scale and velocity. If GSAP's
     * Physics2DPlugin is available, it is used for physics-based motion; otherwise a fallback
     * eased translation is used. Particles are removed from the DOM when their animation completes.
     *
     * @param {HTMLElement} parent - The element to append particle dots to.
     * @param {number} quantity - Number of particles to generate.
     * @param {number} x - Starting x-coordinate (pixels) for each particle.
     * @param {number} y - Starting y-coordinate (pixels) for each particle.
     * @param {number} minAngle - Minimum emission angle in degrees.
     * @param {number} maxAngle - Maximum emission angle in degrees.
     */
