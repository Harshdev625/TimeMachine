document.addEventListener("DOMContentLoaded", function () {
  ...
  const animateElements = document.querySelectorAll(
    ".feature-card, .screenshot-item, .section-header"
  );
  ...
  const carousel = {
    ...
    setupInfiniteLoop() {
      ...
      this.container.style.transform = `translateX(-${
        this.currentSlide * this.slideWidthPercent
      }%)`;
    },
    ...
    recomputeWidths() {
      ...
      this.container.style.transition =
        prevTransition || "transform 0.6s cubic-bezier(0.25, 1, 0.5, 1)";
    },
...
document.querySelectorAll(".btn").forEach((button) => {
  ...
  const svgPath = new Proxy(
    { y: null },
    {
      set(target, key, value) {
        target[key] = value;
        if (target.y !== null) {
          line.innerHTML = getPath(target.y, 0.25);
        }
        return true;
      },
    }
  ...
  tl.to(svgPath, { y: 15, duration: 0.15 }, 0.65).to(
    svgPath,
    { y: 12, ease: "elastic.out(1.2, .7)", duration: 0.6 },
    0.8
  );
